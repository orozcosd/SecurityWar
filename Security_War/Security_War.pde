...
void setup() {
  ...
  // Fuente pixel art
  pixelFont = createFont("Pixellari.ttf", 32);
  textFont(pixelFont);
  noSmooth();
...
void draw() {
  ...
  switch (gameState) {
    case 0: // MENU PRINCIPAL
      ...
      imageMode(CORNER);
      image(btnJugar, width / 2 - 100, 270, 200, 60);
      image(btnAjustes, width / 2 - 100, 350, 200, 60);
      image(btnSalir, width / 2 - 100, 430, 200, 60);
...
void mousePressed() {
  // Menú principal
  if (gameState == 0) {
    if (overButton(width / 2 - 100, 250, 200, 60)) {
      gameState = 1;
      return;
    }
    if (overButton(width / 2 - 100, 330, 200, 60)) {
      previousState = gameState;
      gameState = 4;
      return;
    }
    if (overButton(width / 2 - 100, 410, 200, 60)) {
      exit();
      return;
    }
